...
class CanonicalConditionBreakoutLong(IStrategy):
    timeframe = "15m"
    can_short = False
    startup_candle_count = 200
    stoploss = -0.025
    minimal_roi = {"0": 0.009, "240": 0.004, "720": 0.0}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["rolling_high20"] = dataframe["high"].rolling(20).max().shift(1)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        long_entries = (
            (dataframe["close"] > dataframe["rolling_high20"])
            & (dataframe["close"].shift(1) <= dataframe["rolling_high20"].shift(1))
            & (dataframe["ema50"] > dataframe["ema50"].shift(4))
            & (dataframe["ema20"] > dataframe["ema50"])
            & (dataframe["volume"] >= dataframe["volume_mean20"] * 1.05)
