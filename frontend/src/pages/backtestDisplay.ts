...
export function formatNumber(value: number | null, suffix = ""): string {
  return value === null ? NONE_TEXT : `${value.toFixed(2)}${suffix}`;
}

export function formatInteger(value: number | null): string {
  return value === null ? NONE_TEXT : String(value);
}
...
export function buildBacktestMatrixSummary(
  ...
  for (const row of rows) {
    ...
    if (row.reason !== NONE_TEXT && (row.status === "FAILED" || row.status === "BLOCKED")) {
  ...
  return {
    ...
    statusCounts,
    metricRanges: [
      metricRange("收益率", rows.map((row) => row.metrics.profitPct), "%"),
      metricRange("最大回撤", rows.map((row) => row.metrics.maxDrawdownPct), "%"),
      metricRange(
        "胜率",
