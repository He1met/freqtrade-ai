...
const REASON: Readonly<Record<string, GuidanceDefinition>> = {
  ...
  RESEARCH_CONTEXT_CONFLICT: guidance("研究阶段证据冲突", "所选策略、计划或阶段字段无法组成一致的顺序与 lineage；后续步骤保持未知。", "查看研究诊断", "/v13/research", "danger"),
  RESEARCH_CONTEXT_UNSELECTED: guidance("尚未选择精确研究计划", "当前没有 exact validation plan 上下文；页面不会从策略摘要推断中间步骤。", "进入研究状态", "/v13/research", "warning"),
  ...
  CANONICAL_ID_GENERATION_UNAVAILABLE: guidance("无法生成请求标识", "当前浏览器不支持安全的随机 UUID；页面不会使用可预测值替代。", "检查浏览器环境", "/v13/submission", "danger"),
  SELECTED_CONTEXT_NOT_FOUND: guidance("所选范围已失效", "API 返回的 Scope/Workflow 选项中没有当前 URL 对象；页面不会切换到首个选项。", "重新选择研究范围", "/v13/configuration", "warning"),
  SELECTED_MARKET_PROFILE_NOT_FOUND: guidance("所选行情配置已失效", "API 返回的行情配置中没有当前 URL 对象；页面不会使用其他配置替代。", "重新选择行情配置", "/v13/market-data", "warning"),
  SELECTED_PLAN_NOT_FOUND: guidance("所选验证计划已失效", "API 返回的验证计划中没有当前 URL 对象或其 lineage 不匹配；页面保持阻断。", "重新选择验证计划", "/v13/research", "warning"),
  SELECTED_PROFILE_NOT_FOUND: guidance("所选配置 Profile 已失效", "API 返回的配置 Profile 中没有当前 URL 对象；页面不会自动替换。", "重新选择配置 Profile", "/v13/configuration", "warning"),
  SELECTED_SNAPSHOT_NOT_FOUND: guidance("所选行情快照已失效", "API 返回的行情快照中没有当前 URL 对象；页面不会自动替换。", "重新选择行情快照", "/v13/market-data", "warning"),
  SELECTED_STRATEGY_NOT_FOUND: guidance("所选策略已失效", "API 返回的策略目录中没有当前 URL 对象；页面不会从详情响应反推可选项。", "返回策略目录重新选择", "/v13/strategies", "warning"),
  SELECTED_TARGET_NOT_FOUND: guidance("所选研究目标已失效", "API 返回的当前 lineage 中没有 URL 指定目标；页面不会切换到其他目标。", "重新选择研究目标", "/v13/research", "warning"),
  SELECTED_VERSION_NOT_FOUND: guidance("所选配置版本已失效", "API 返回的配置版本中没有当前 URL 对象；页面不会自动替换。", "重新选择配置版本", "/v13/configuration", "warning"),
