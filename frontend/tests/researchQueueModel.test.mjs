...
function item(overrides = {}) {
  return { candidate_id: "candidate-1", candidate_name: "MomentumOne", pair: "BTC/USDT:USDT", timeframe: "5m", generated_at: "2026-08-11T01:00:00Z", queue_position: 1, status: "WAITING_FOR_LEASE", current_step: "等待领取", completed_steps: ["候选已持久化"], next_step: "领取 lease", progress_percent: null, started_at: null, completed_at: null, elapsed_seconds: null, preceding_count: 0, attempt: 0, reason_code: "AWAITING_VALIDATION_LEASE", reason_message: "等待串行 worker 领取", evidence: [], actions: { cancel_available: false, retry_available: false, reason_code: "READ_ONLY" }, ...overrides };
}

test("NO_ACTION is neutral while FAILED and REJECTED have different advice", () => {
  assert.equal(researchQueueStatusLabel("NO_ACTION"), "无动作");
  assert.equal(researchQueueStatusTone("NO_ACTION"), "neutral");
...
test("filters cover status, pair, timeframe and batch without hiding defaults", () => {
  const rows = [item({ candidate_id: "b", candidate_name: "Beta", pair: "ETH/USDT:USDT", queue_position: 2, status: "REJECTED" }), item({ candidate_id: "a", candidate_name: "Alpha", queue_position: 1, status: "QUALIFIED" })];
  ...
  assert.deepEqual(filterAndSortQueueItems(rows, { ...all, status: "QUALIFIED", batch: "run" }, "name", "run").map((row) => row.candidate_id), ["a"]);
  assert.deepEqual(terminalGroups(rows).map((group) => [group.status, group.items.length]), [["QUALIFIED", 1], ["REJECTED", 1], ["FAILED", 0], ["NO_ACTION", 0]]);
});
