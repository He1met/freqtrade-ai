...
test("missing queue API never fabricates active, waiting, lease, progress or terminal status", () => {
  ...
  assert.equal(projection.batch.expected_count, 37);
  assert.equal(projection.completed[0].status, "UNKNOWN");
  assert.equal(projection.completed[0].current_step, "历史研究状态（非队列投影）");
  assert.equal(projection.completed[0].progress_percent, null);
  assert.deepEqual(terminalGroups(projection.completed).find((group) => group.status === "UNKNOWN").items, projection.completed);
});
...
test("filters cover status, pair, timeframe and batch without hiding defaults", () => {
  ...
  assert.deepEqual(terminalGroups(rows).map((group) => [group.status, group.items.length]), [["VALIDATED", 1], ["REJECTED", 1], ["FAILED", 0], ["DEPLOYED", 0], ["UNKNOWN", 0]]);
});
