...
| Demo 策略选择、pair→instrument、slot | `okx_demo_strategy_selection.py`、`strategy_deployment_continuation.py`、`configure_okx_demo_continuous.py` 固定 map/名称/最低分/slot | `strategy_target_id`、deployment selector、capacity profile、promotion/quality profile | `PROTECTED_RUNTIME + BLOCKED_CONFIGURATION_CONTRACT`。不得修改 OKX/deployment/order 路径，也不得迁移旧名称 allowlist。 |
