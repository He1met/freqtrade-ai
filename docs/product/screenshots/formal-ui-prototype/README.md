...
当前本地正式数据库仍为 schema v36，尚无 v37 的 append-only research receipt 表。因此研究 workspace 投影读取失败时，页面按设计显示“未知”，没有用 fixture、mock 或历史批次推断为成功。OKX_DEMO 部署、信号、订单和对账区块来自可用的真实只读 API。
...
移动端、平板和窄屏响应式已从当前产品范围与验收门禁移除；既有多视口自动化仅保留为历史回归证据。
