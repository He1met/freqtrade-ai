...
# Planless static/lookahead gate evidence. Attempts have a bounded mutable
# lifecycle for lease recovery; terminal receipts are insert-only in the ACL.
RESEARCH_GATE_ATTEMPTS_TABLE = _table(
    "research_gate_attempts",
    _uuid_id(),
    _uuid_fk("strategy_version_id", "strategy_versions"),
    _uuid_fk("research_target_id", "research_targets"),
    _digest("artifact_digest"),
    Column("gate_contract_version", String(80), nullable=False),
    Column("release_commit", String(40), nullable=False),
    _digest("executor_image_digest"),
    _digest("worker_source_digest"),
    _uuid_fk("target_snapshot_id", "configuration_snapshots"),
    _digest("target_snapshot_digest"),
    _uuid_fk("window_snapshot_id", "configuration_snapshots"),
    _digest("window_snapshot_digest"),
    _uuid_fk("market_profile_version_id", "market_profile_versions"),
    _digest("market_profile_digest"),
    _uuid_fk("configuration_bundle_id", "configuration_bundles"),
    _digest("configuration_bundle_digest"),
    _uuid_fk("market_snapshot_id", "market_snapshots"),
    _digest("market_snapshot_digest"),
    ...
    _digest("request_digest"),
    Column("status", String(16), nullable=False),
    Column("terminal_reason_code", String(120), nullable=True),
    Column("writer_identity", String(200), nullable=False),
    _digest("lease_token_digest", nullable=True),
    _created_at("lease_expires_at", nullable=True),
    _created_at(),
...
RESEARCH_GATE_RECEIPTS_TABLE = _table(
    ...
    _uuid_id(),
    _uuid_fk("gate_attempt_id", "research_gate_attempts"),
    Column("gate_type", String(16), nullable=False),
    Column("gate_contract_version", String(80), nullable=False),
    _uuid_fk("strategy_version_id", "strategy_versions"),
    _uuid_fk("research_target_id", "research_targets"),
    _digest("artifact_digest"),
    ...
    _digest("market_snapshot_digest"),
    _digest("request_digest"),
    Column("terminal_status", String(16), nullable=False),
    Column("reason_code", String(120), nullable=True),
    Column("failure_stage", String(40), nullable=True),
    Column("tool_return_code", Integer, nullable=True),
    _digest("evidence_digest"),
    _digest("stdout_digest"),
    _digest("stderr_digest"),
    Column("evidence_json", JSON, nullable=False),
    Column("observed_signal_count", Integer, nullable=True),
    Column("observed_trade_count", Integer, nullable=True),
...
VALIDATION_PLANS_TABLE = _table(
    ...
    *_lineage_columns(include_plan=False),
    _uuid_fk("window_snapshot_id", "configuration_snapshots"),
    _uuid_fk("static_gate_receipt_id", "research_gate_receipts", nullable=True),
    _uuid_fk("lookahead_gate_receipt_id", "research_gate_receipts", nullable=True),
    _digest("validation_plan_digest"),
    Column("status", String(16), nullable=False),
    _created_at(),
    UniqueConstraint(
