...
def record_simulated_fill(
    ...
    *,
    ...
    order = effective.execute(
        select(ORDERS_TABLE).where(ORDERS_TABLE.c.id == order_id)
    ).mappings().one_or_none()
    if order is None or order["status"] != "ACCEPTED" or order["receipt_digest"] is None:
    ...
    existing = effective.execute(
        select(FILLS_TABLE).where(FILLS_TABLE.c.exchange_fill_id == exchange_fill_id)
    ).mappings().one_or_none()
    if existing is not None:
        if existing["order_id"] != order_id or existing["receipt_digest"] != receipt_digest:
            raise CanonicalExecutionChainBlocked(
