...
def test_phase9_postgresql_upgrade_rollback_and_exact_replay() -> None:
    ...
    try:
        with engine.begin() as connection:
            ...
            upgraded = apply_phase9_schema_upgrade(connection)
            ...
            repeated = apply_phase9_schema_upgrade(connection)
            assert repeated.status == "ACCEPTED"
            assert repeated.repeat_noop is True
            assert repeated.receipt_digest == verify_phase9_schema_upgrade(
                connection
            ).receipt_digest
    finally:
        engine.dispose()
