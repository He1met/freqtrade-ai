...
def test_running_gate_replay_reaches_claim_for_expired_lease_recovery(
    monkeypatch, tmp_path: Path
) -> None:
    attempt_id = uuid4()
    calls = []

    def request(_environment, *, method, path, payload):
        calls.append((method, path, payload))
        if path == "/research/gates/attempts":
            return {
                "status": "RUNNING",
                "repeat_noop": True,
                "gate_attempt_id": str(attempt_id),
            }
        return {"status": "BLOCKED", "reason_code": "BLOCKED_GATE_LEASE_ACTIVE"}

    reader = next(
        principal
        for principal, capability in cli.LOCAL_SERVICE_PRINCIPALS.items()
        if capability == "canonical_api_reader"
    )
    environment = {
        cli.READER_DATABASE_URL_ENV: (
            f"postgresql+psycopg://{reader}@localhost/freqtrade_ai_v13"
        ),
        cli.MARKET_ROOT_ENV: str(tmp_path),
        cli.WORKSPACE_ROOT_ENV: str(tmp_path),
        cli.CPU_LIMIT_ENV: "1",
        cli.MEMORY_LIMIT_ENV: "1024",
        cli.TIMEOUT_LIMIT_ENV: "60",
        cli.OUTPUT_LIMIT_ENV: "1024",
        cli.PIDS_LIMIT_ENV: "64",
        cli.TMPFS_LIMIT_ENV: "64",
    }
    lineage = {
        "strategy_version_id": str(uuid4()),
        "research_target_id": str(uuid4()),
        "configuration_bundle_id": str(uuid4()),
        "configuration_bundle_digest": "a" * 64,
        "market_snapshot_id": str(uuid4()),
        "market_snapshot_digest": "b" * 64,
    }
    monkeypatch.setattr(cli, "_verify_gate_release_identity", lambda *_args: None)
    monkeypatch.setattr(cli, "_request", request)
    result = cli._gate_execute(
        environment,
        {
            "lineage": lineage,
            "idempotency_key": "recovery",
            "release_commit": "c" * 40,
            "executor_image_digest": "d" * 64,
            "worker_source_digest": "e" * 64,
        },
    )
    assert result["reason_code"] == "BLOCKED_GATE_LEASE_ACTIVE"
    assert calls[1][1] == f"/research/gates/attempts/{attempt_id}/claim"
